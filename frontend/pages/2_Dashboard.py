...
[data-testid="stAppViewContainer"] {
    background: radial-gradient(ellipse 70% 40% at 50% -5%, rgba(220,38,38,0.12) 0%, #0D0F14 55%);
}
...
st.markdown('<p class="hero-subtitle">Real-time overview of all prediction results and patient risk trends.</p>', unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
try:
    history = requests.get("http://127.0.0.1:5000/history")
    data = history.json()
except Exception:
    data = []
...
if data:
    ...
    with k2:
        ...
        <div class="kpi-card high">
            ...
            <div class="kpi-sub">{high/total*100:.0f}% of total</div>
        </div>""", unsafe_allow_html=True)
    ...
    with k3:
        ...
        <div class="kpi-card mod">
            ...
            <div class="kpi-sub">{moderate/total*100:.0f}% of total</div>
        </div>""", unsafe_allow_html=True)
    ...
    with k4:
        ...
        <div class="kpi-card low">
            ...
            <div class="kpi-sub">{low/total*100:.0f}% of total</div>
        </div>""", unsafe_allow_html=True)
    ...
    with k5:
        ...
        <div class="kpi-card total">
            ...
            <div class="kpi-sub">Population mean</div>
        </div>""", unsafe_allow_html=True)
    ...
    with c2:
        ...
        # Scatter colored by risk category
        ...
        fig_line.add_trace(go.Scatter(
            ...
            mode="lines",
            name="Trend",
            line=dict(color='rgba(255,255,255,0.12)', width=1.5, dash='solid'),
            showlegend=False,
            hoverinfo='skip'
        ))
        ...
        fig_line.update_layout(
            **PLOT_CONFIG,
            ...
            yaxis=dict(title="Risk Score", tickformat=".0%", **GRID_STYLE,
    ...
    if "age" in df.columns:
        with d1:
            ...
            st.markdown('<p class="section-sub">Correlation between patient age and cardiac probability.</p>', unsafe_allow_html=True)
    ...
    if "chol" in df.columns:
        with d2:
            ...
            st.markdown('<p class="section-sub">Cholesterol levels segmented by risk category.</p>', unsafe_allow_html=True)
    ...
    st.markdown('<p class="section-sub">Full log of all patient predictions. Click column headers to sort.</p>', unsafe_allow_html=True)
