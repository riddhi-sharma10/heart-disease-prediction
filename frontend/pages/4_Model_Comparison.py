...
with ch1:
    ...
    fig_acc.update_layout(
        **PLOT_CFG, height=300,
        ...
        yaxis=dict(**GRID, ticksuffix="%", range=[
            max(0, df["Accuracy"].min() * 100 - 5), 100
...
with ch2:
    ...
    fig_auc.update_layout(
        **PLOT_CFG, height=300,
        ...
        xaxis=dict(**GRID, tickfont=dict(size=12, color='#94A3B8')),
        yaxis=dict(**GRID, ticksuffix="%", range=[
            max(0, df["ROC_AUC"].min() * 100 - 5), 100
        ], tickfont=dict(size=11)),
    )
...
for _, row in df.iterrows():
    ...
    fig_radar.add_trace(go.Scatterpolar(
        ...
        fillcolor=hex_to_rgba(row["Color"], 0.08),  # ✅ FIXED
    ...
    **PLOT_CFG,
    ...
    polar=dict(
        ...
        radialaxis=dict(
            visible=True,
            range=[
                max(0, min(df["Accuracy"].min(), df["ROC_AUC"].min()) * 100 - 5),
                100
            ],
...
st.markdown(f'<p class="section-header">Model Rankings</p>', unsafe_allow_html=True)
...
for i, row in df_ranked.iterrows():
    ...
    <div class="rank-row">
        ...
        <div class="rank-bar-wrap">
            <div class="rank-bar" style="width:{bar_pct:.1f}%; --bar-color:{color};
                 background:{color};"></div>
        </div>
        <div class="rank-val" style="--bar-color:{color}; color:{color};">
            {val*100:.2f}%
        </div>
    </div>
    """, unsafe_allow_html=True)