...
def hex_rgba(hex_color, alpha):
    """Safe hex to rgba — avoids the broken .replace('#','rgba(') pattern."""
    h = hex_color.lstrip('#')
...
with dist_col:
    ...
    colors_hist      = [risk_color(c) for c in centers]

    fig_hist = go.Figure()
    for i, (count, center, color) in enumerate(zip(counts, centers, colors_hist)):
...
with pct_col:
    ...
    # ✔ FIX: guard against NaN std when only 1 record
    std_val  = df["risk_percent"].std()
    std_disp = f"{std_val:.1f}%" if not np.isnan(std_val) else "N/A"
    ...
    stats = [
        ...
        ("Maximum",  df["risk_percent"].max(),              "#F87171"),
    ]
    ...
    # Std Dev shown separately with NaN guard
    st.markdown(f"""
    <div style="display:flex; justify-content:space-between; align-items:center;
                padding: 10px 16px; margin-bottom:8px;
                background:rgba(255,255,255,0.025);
                border:1px solid rgba(255,255,255,0.06);
                border-radius:10px;">
        <span style="font-size:12px; color:#475569; font-weight:500;">Std Dev</span>
        <span style="font-size:15px; font-weight:600; color:#A78BFA;">{std_disp}</span>
    </div>""", unsafe_allow_html=True)
...
with box_col:
    ...
    for label, color in [("Low","#34D399"), ("Moderate","#FBBF24"), ("High","#F87171")]:
        ...
        fig_box.add_trace(go.Box(
            y=sub, name=label,
            marker=dict(color=color, size=5, line=dict(color='#0D0F14', width=1)),
            line=dict(color=color),
            # ✔ FIX: use proper hex_rgba() instead of broken .replace() hack
